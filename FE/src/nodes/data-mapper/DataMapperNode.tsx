...
import { Split, Play, Trash2 } from 'lucide-react';
...
export const DataMapperNode = memo(({ id, data, isConnectable, selected }: NodeProps) => {
    ...
    const getStatusColor = () => {
        if (isRunning) return 'border-pink-400 shadow-[0_0_15px_rgba(236,72,153,0.5)]';
        ...
        return selected ? 'border-pink-500 ring-2 ring-pink-100 shadow-lg' : 'border-slate-200 hover:border-pink-400 shadow-sm';
    };
    ...
    return (
        <div className={`relative group min-w-[220px] bg-white rounded-xl border-2 transition-all duration-300 ${getStatusColor()}`}>
            ...
            {/* Header */}
            <div className="flex items-center justify-between px-3 py-2 bg-gradient-to-r from-pink-50 to-white border-b border-pink-100/50 rounded-t-[10px]">
            ...
            {(isRunning || isSuccess || isFailed) && (
             ...
             {/* Loading Overlay */}
             {isRunning && (
                <div className="absolute inset-0 bg-pink-50/10 backdrop-blur-[0.5px] rounded-xl animate-pulse pointer-events-none" />
            )}
            ...
            <Handle 
                ...
                className="!w-2 !h-4 !bg-slate-400 !border-2 !border-white !rounded-sm transition-all hover:!bg-pink-500 shadow-sm" 
            />
            <Handle 
                type="source" 
